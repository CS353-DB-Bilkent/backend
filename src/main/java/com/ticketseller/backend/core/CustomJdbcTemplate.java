...
public class CustomJdbcTemplate {
    ...
    public <T> List<T> query(String sql, CustomSqlParameters params, RowMapper<T> rowMapper) {
        try {
            ...
            throw new BaseRuntimeException("Error: " + e.getLocalizedMessage(), 0, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public <T> T queryForObject(String sql, CustomSqlParameters params, RowMapper<T> rowMapper) {
        try {
            ...
            throw new DatabaseRuntimeException("Error: " + e.getLocalizedMessage(), 0, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    private void checkParams(String sql, MapSqlParameterSource params) {
        ...
        List<SqlParameter> sqlStringParameterList =
                NamedParameterUtils.buildSqlParameterList(NamedParameterUtils.parseSqlStatement(sql),
